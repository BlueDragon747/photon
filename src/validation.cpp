...
#if defined(NDEBUG)
# error "Photon cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Photon Signed Message:\n";
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    // Photon's legacy 0.8 chain intentionally only enforced the BIP30
    // overwrite check for CreateNewBlock-style contexts (where pindex has no
    // hash yet). Preserve that historical behavior here so replaying older
    // Photon chain data does not hit a broad Bitcoin-style duplicate-txid
    // rejection during IBD.
    //
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime, bool fCheckPOW = true)
{
    ...
    // Photon historical data carries AuxPoW data before the nominal
